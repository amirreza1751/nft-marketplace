...
export class BuyNowService implements OnApplicationBootstrap {
  ...
  async listenBuyNowItemCreated() {
    ...
    this.marketContract.events
      .BuyNowItemCreated()
      .on('data', async (buynowItemCreatedEvent) => {
  ...
  async doListenBuyNowItemCreated(buyNowItemCreatedEvent) {
    console.log(
      'Buy now item created: ' + buyNowItemCreatedEvent.returnValues.itemId,
    );
    let seller = await this.userService.findOrCreateByAddress(
      buyNowItemCreatedEvent.returnValues.seller,
    );
    ...
    let currency = await this.erc20Service.findOrCreateByAddress(
      buyNowItemCreatedEvent.returnValues.currency,
    );
    let buyNowItem = await this.findOrCreateByItemId(
      buyNowItemCreatedEvent.returnValues.itemId,
    );
  ...
  async listenBuyNowItemUpdated() {
    ...
    this.marketContract.events
      .BuyNowItemUpdated()
      .on('data', async (buyNowItemUpdatedEvent) => {
  ...
  async doListenBuyNowItemUpdated(buyNowItemUpdatedEvent) {
    let buyNowItem = await this.findOrCreateByItemId(
      buyNowItemUpdatedEvent.returnValues.itemId,
    );
  ...
  async listenBuyNowItemPurchased() {
    ...
    this.marketContract.events
      .BuyNowItemPurchased()
      .on('data', async (buyNowItemPurchasedEvent) => {
  ...
  async doListenBuyNowItemPurchased(buyNowItemPurchasedEvent) {
    let buyNowItem = await this.findOrCreateByItemId(
      buyNowItemPurchasedEvent.returnValues.itemId,
    );
    let winner = await this.userService.findOrCreateByAddress(
      buyNowItemPurchasedEvent.returnValues.winner,
    );
  ...
  async listenBuyNowItemCanceled() {
    ...
    this.marketContract.events
      .BuyNowItemCanceled()
      .on('data', async (buyNowItemCanceledEvent) => {
